...
bool Parser::importMesh(const aiMesh * mesh, ObjectGroup * og, Material * mat){
  ...
  for(u_int j = 0; j < nFaces; ++j){
    ...
    // }
      ...
      Vec3d up = fabs(n0.z) < 1.-D_EPS ? Vec3d(0.0,0.0,1.0) : Vec3d(1,0,0);
      tn0 = (up^n0).hat();
      up = fabs(n1.z) < 1.-D_EPS ? Vec3d(0.0,0.0,1.0) : Vec3d(1,0,0);
      tn1 = (up^n1).hat();
    ...
    og->addObject(new Triangle(p0, p1, p2, n0, n1, n2, t0, t1, t2, tn0, tn1, tn2, mat));
  }
  return true;
}
...
bool Parser::traverseMaterialsNode(xml_node<> * node){
  ...
  if((attr = node->first_attribute("emis")) && strcmp(attr->value(),"") != 0){
    ...
    if(val.size() < 2)
      ...
      emis = Vec3d(stod(val[0], nullptr),stod(val[1], nullptr),stod(val[2], nullptr));
  }
  ...
  if((attr = node->first_attribute("type")) && strcmp(attr->value(),"") != 0){
    if(strcmp(attr->value(),"lambert") == 0){
      ...
      if((attr = node->first_attribute("sMap")) && strcmp(attr->value(),"") != 0){
        if((txt = new Texture(attr->value()))){
          if((attr = node->first_attribute("rMap")) && strcmp(attr->value(),"") != 0){
            Texture * txt2;
            if((txt2 = new Texture(attr->value()))){
              if(res.size() > 1)
                mBrdf->addBrdf(new CookTorrance(txt, txt2, stod(res[0],nullptr), stod(res[1],nullptr)));
              else
                mBrdf->addBrdf(new CookTorrance(txt, txt2, 1., 1.));
            }
          }
          else if(res.size() > 1 ){
            mBrdf->addBrdf(new CookTorrance(txt, stod(res[0],nullptr), stod(res[1],nullptr)));
          }
          else if(res.size() > 0 ){
            mBrdf->addBrdf(new CookTorrance(txt, stod(res[0],nullptr), 1.));
          }
          else{
            std::cout << "[Warning] <" << node->name() << " \"roughness\"> value required! Used default value.\n";
            mBrdf->addBrdf(new CookTorrance(txt,1, 1));
          }
        }
        else{
          std::cout << "[Warning] <" << node->name() << " \"sMap\"> attribute error!.\n";
        }
      }
      else if((attr = node->first_attribute("rMap")) && strcmp(attr->value(),"") != 0){
        if((txt = new Texture(attr->value()))){
          if((attr = node->first_attribute("sMap")) && strcmp(attr->value(),"") != 0){
            Texture * txt2;
            if((txt2 = new Texture(attr->value()))){
            ...
            mBrdf->addBrdf(new CookTorrance(stod(res[0],nullptr), txt, stod(res[1],nullptr)));
          }
          else if(res.size() > 0 ){
          ...
          std::cout << "[Warning] <" << node->name() << " \"rMap\"> attribute error!.\n";
        }
      }
      else if(res.size() > 3 ){
        mBrdf->addBrdf(new CookTorrance(Vec3d(stod(res[0],nullptr),stod(res[1],nullptr),stod(res[2],nullptr)),stod(res[3],nullptr)));
      }
      else if(res.size() > 1 ){
        mBrdf->addBrdf(new CookTorrance(Vec3d(stod(res[0],nullptr)),stod(res[1],nullptr)));
      }
      else{
        std::cout << "[Warning] <" << node->name() << " \"specular,roughness\"> values size != 4! Used default value.\n";
